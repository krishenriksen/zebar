...
use tracing::info;
use windows::Win32::{
    Foundation::{HWND, RECT, LPARAM, WPARAM, LRESULT, GetLastError},
    UI::Shell::{
        SHAppBarMessage, ABE_BOTTOM, ABE_LEFT, ABE_RIGHT, ABE_TOP, ABM_NEW,
        ABM_QUERYPOS, ABM_REMOVE, ABM_SETPOS, APPBARDATA, ABN_POSCHANGED,
    },
    UI::WindowsAndMessaging::{
        SetWindowLongPtrA, GWLP_WNDPROC, CallWindowProcA, WNDPROC,
        DefWindowProcA, GetWindowRect, RegisterWindowMessageA
    }
};
...
fn update_app_bar_position(hwnd: HWND, edge: DockEdge) -> anyhow::Result<()> {
...
pub fn create_app_bar(
    ...
    // Subclass the window to receive messages. Store the original window proc.
    unsafe {
